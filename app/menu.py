...
class LogMenu(BaseMenu):
    '''Simple menu class that prints notifications to the console instead of using curses.'''
    def run(self, config: ConfigManager, dispatcher: Dispatcher, profile: Profile, scheduler: Scheduler, 
            threads: list[Thread]) -> None:
        '''Starts the log menu.'''
        self.config = config
        ...
        print(f'[*] LogMenu started. (Press Ctrl+C to exit)')
        
        while self.is_alive:
            for thread in threads:
                if not thread.is_alive():
                    self.kill()
                    print(f'[E] "{thread.name}" thread exited.')
                    return None
            sleep(1)
