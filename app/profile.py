...
class Profile:
    ...
    def update(self, raw_data: str) -> bool:
        ...
        for line in remove_markdown(raw_data).split('\n'):
            ...
            if line.startswith('Balance:'):
                ...
                self.balance_num = numeric_balance
                self.balance = '${:,.2f}'.format(numeric_balance)
                # Removed total balance logging
    ...
    def receive_sell_message(self, message: str) -> bool:
        '''Parses the gold earned from a sell message and updates the balance.'''
        try:
            # Flexible parsing for: You sold your entire inventory for $204. You now have $31,225!
            if 'sold' in message.lower() and '$' in message:
                import re
                prices = re.findall(r'\$[\d,]+', message)
                if len(prices) >= 2:
                    # First price is earned, second is total
                    earned_str = prices[0].replace('$', '').replace(',', '')
                    total_str = prices[1].replace('$', '').replace(',', '')
                    
                    gold_earned = int(earned_str)
                    new_balance = int(total_str)
